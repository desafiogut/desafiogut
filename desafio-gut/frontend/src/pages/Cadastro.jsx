...
// MC91.7 — Cadastro do usuário comum por e-mail (OTP), reutilizando o padrão
// headless do SejaNossoParceiro (MC15.6: useLoginWithEmail / sendCode /
// loginWithCode). O cadastro efetivo é a criação da identidade Privy no
// primeiro login (e-mail + carteira embedded + auth-user). Nome e telefone
// são coletados na UI para completar o fluxo pedido, mas NÃO são persistidos:
// a infra atual não tem perfil de usuário comum (R1 — reutilizar, não criar
// backend novo). O login reutiliza exatamente a lógica existente.
...
export default function Cadastro() {
  ...
  // Redirect reativo após o login (mesmo padrão do corporativo, MC17).
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: CORPO.bg, color: CORPO.text, padding: "1.25rem", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div className="gut-glass-standard" style={{ width: "100%", maxWidth: 420, padding: "1.5rem", borderRadius: "18px" }}>
        <h1 style={{ margin: "0 0 0.25rem", fontSize: "1.25rem", fontWeight: 800 }}>⚡ Cadastro</h1>
        <p style={{ margin: "0 0 1.25rem", color: CORPO.muted, fontSize: "0.82rem" }}>
          {etapa === "form"
            ? "Primeiro acesso? Crie sua conta com nome, e-mail e telefone."
        ...
        {etapa === "form" ? (
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
            <div>
              <label style={labelStyle} htmlFor="cad-nome">Nome</label>
              <input id="cad-nome" style={inputStyle} value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Seu nome" autoComplete="name" />
            </div>
            <div>
              <label style={labelStyle} htmlFor="cad-email">E-mail</label>
              <input id="cad-email" type="email" style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="voce@email.com" autoComplete="email" />
            </div>
            <div>
              <label style={labelStyle} htmlFor="cad-telefone">Telefone (opcional)</label>
              <input id="cad-telefone" type="tel" style={inputStyle} value={telefone} onChange={(e) => setTelefone(e.target.value)} placeholder="(00) 00000-0000" autoComplete="tel" />
            </div>
          ...
          <form onSubmit={handleConfirmar} style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
            <div>
              <label style={labelStyle} htmlFor="cad-codigo">Código de verificação</label>
              <input id="cad-codigo" style={inputStyle} value={codigo} onChange={(e) => setCodigo(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="••••••" inputMode="numeric" autoFocus />
            </div>
            <Button type="submit" size="lg" disabled={enviando} className="w-full">
              {enviando ? "⏳ Entrando…" : "✅ Confirmar e entrar"}
            </Button>
        ...
        <div style={{ marginTop: "1.25rem", textAlign: "center", fontSize: "0.82rem", color: CORPO.muted }}>
          Já tem conta?{" "}
          <Link to="/login-email" style={{ color: COR.accent, fontWeight: 700 }}>Entrar com e-mail</Link>
        </div>
      </div>
    </div>
  );
