...
//
// Acessível em /admin. Gate frontend: apenas endereços listados em
// admin-list ou a coordenação. Operações de mutação exigem x-admin-token
// — pedimos uma vez por sessão e guardamos em sessionStorage.
//
// Tabs:
//   1. Aprovações: lista pedidos pendentes/aprovados/rejeitados
//   2. Cotas:     lista cotas por categoria + admite criar/atualizar
//   3. Admins:    lista admins atuais + adicionar/remover
...
const TOKEN_SS_KEY = "gut_admin_token";

function lerToken()  { try { return sessionStorage.getItem(TOKEN_SS_KEY) || ""; } catch { return ""; } }
function gravarToken(t) { try { if (t) sessionStorage.setItem(TOKEN_SS_KEY, t); else sessionStorage.removeItem(TOKEN_SS_KEY); } catch {} }
...
function TabAprovacoes({ token, isMobile, onTokenMissing }) {
...
function TabCotas({ token, isMobile, onTokenMissing }) {
...
function TabAdmins({ token, onTokenMissing }) {
...
export default function AdminPanel() {
  ...
  const { address, abrirModal, isConnected } = useAppContext();
  const { isAdmin, loading } = useAdmin(address);
  const [aba, setAba] = useState("aprovacoes");
  const [token, setToken] = useState(() => lerToken());
  const [pedindoToken, setPedindoToken] = useState(false);
  const [tokenInput, setTokenInput] = useState("");

  function onTokenMissing() {
    setPedindoToken(true);
  }
  function salvarToken(e) {
    e.preventDefault();
    setToken(tokenInput);
    gravarToken(tokenInput);
    setPedindoToken(false);
    setTokenInput("");
  }
  function limparTokenSession() {
    setToken("");
    gravarToken("");
  }
  ...
  return (
    <div style={{ padding: isMobile ? "1rem" : "1.5rem 2rem", color: COR.text, display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{
        ...
        background: token ? "rgba(16,185,129,0.06)" : "rgba(251,191,36,0.06)",
        border: `1px solid ${token ? "rgba(16,185,129,0.3)" : "rgba(251,191,36,0.3)"}`,
        borderRadius: "10px",
        fontSize: "0.78rem", color: COR.text,
        display: "flex", flexWrap: "wrap", gap: "0.6rem", alignItems: "center", justifyContent: "space-between",
      }}>
        <span>{token ? "✓ Admin token na sessão" : "⚠ Sem admin token nesta sessão (necessário para mutações)"}</span>
        {token ? (
          <button onClick={limparTokenSession}
          ...
          <button onClick={() => setPedindoToken(true)}
            style={{ padding: "0.28rem 0.7rem", background: COR.warn, color: "#0a0f1a", border: "none", borderRadius: "6px", fontSize: "0.72rem", fontWeight: 800, cursor: "pointer" }}>
            Informar token
          </button>
        )}
      </div>
      {pedindoToken && (
        <form onSubmit={salvarToken} style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
          <input type="password" placeholder="ADMIN_TOKEN" value={tokenInput}
                 onChange={(e) => setTokenInput(e.target.value)} style={{ ...inputStyle, flex: 1, minWidth: 200 }} autoFocus />
          <button type="submit" disabled={!tokenInput}
            style={{ padding: "0.45rem 0.85rem", background: COR.primary, border: "none", borderRadius: "8px", color: "#0a0f1a", fontWeight: 800, cursor: "pointer" }}>Salvar</button>
          <button type="button" onClick={() => setPedindoToken(false)}
      ...
      {/* Conteúdo */}
      <section>
        {aba === "aprovacoes" && <TabAprovacoes token={token} isMobile={isMobile} onTokenMissing={onTokenMissing} />}
        {aba === "cotas"      && <TabCotas      token={token} isMobile={isMobile} onTokenMissing={onTokenMissing} />}
        {aba === "admins"     && <TabAdmins     token={token} onTokenMissing={onTokenMissing} />}
      </section>
    </div>
  );
}
