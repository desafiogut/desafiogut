...
export async function getLances(edicaoId) {
  ...
  const { data, error } = await supabase
    .from(TABELA_LANCES)
    .select("payload")
    .eq("edicao_id", String(edicaoId))
    .order("created_at", { ascending: true });
  if (error) throw new Error(`[data-store-supabase] getLances("${edicaoId}") falhou: ${error.message}`);
  return (data ?? []).map((linha) => linha.payload).filter(Boolean);
}
