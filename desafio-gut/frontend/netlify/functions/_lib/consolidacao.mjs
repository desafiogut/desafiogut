// _lib/consolidacao.mjs — MC94.3. O NÚCLEO da consolidação de uma edição,
// extraído de `consolidar-lances.mjs` (autorização do operador, R18, 2026-09-25)
// para ter DOIS chamadores sem duas cópias do código que move dinheiro on-chain:
//
//   consolidar-lances.mjs            — POST admin (Bearer admin-JWT), como sempre
//   scheduled-encerrar-especial.mjs  — cron, UMA tentativa por edição especial
//
// O código abaixo é o do handler, movido sem mudar a lógica (MC28.1 SEGMENTO 4):
//   1) lê TODOS os lances (Key-Per-Bid) com paginação + paralelismo;
...
//
// ⚠️ AUTORIZAÇÃO NÃO VIVE AQUI. Quem chama já decidiu que pode: o handler com
// guardAdmin, a scheduled por ser código do servidor sem entrada externa.
// Nada neste módulo lê um Request.
//
// Resultado: { status, corpo } (resposta JSON) ou { status, erro:{code,message} }.

import { Contract } from "ethers";
import { marcarConsolidado, estaConsolidado } from "./bids-store.mjs";
import { getLances } from "./data-store.mjs";
import { obterSignerCoordenacao, backendAssinatura } from "./signer.mjs";
...
/** Janela de mineração antes de reportar "pendente" (o handler usa esta). */
export const TIMEOUT_MINER_MS = 90_000;

const falha = (status, code, message) => ({ status, erro: { code, message } });
...
/**
 * Consolida uma edição: apura, assina, envia on-chain, espera o recibo, marca.
 *
 * @param {string} edicaoId
 * @param {{timeoutMinerMs?: number}} [opts] a scheduled passa uma janela curta:
 *   o Netlify corta as scheduled functions aos ~30 s, e 90 s de espera
 *   morreriam a meio, DEPOIS de a transacção ter saído.
 * @returns {Promise<{status:number, corpo?:object, erro?:{code:string,message:string}}>}
 */
export async function consolidarEdicao(edicaoId, { timeoutMinerMs = TIMEOUT_MINER_MS } = {}) {
  if (process.env.NETWORK_STAGE !== "mainnet")
    return falha(409, "fora_de_mainnet", "consolidação só corre em mainnet");
  if (!edicaoId) return falha(400, "edicao_obrigatoria", "edicaoId obrigatório");

  // 3. Idempotência de fecho — não reconsolida
  const jaFechado = await estaConsolidado(edicaoId);
  if (jaFechado) return { status: 200, corpo: { ok: true, idempotent: true, edicaoId, ...jaFechado } };
  ...
  for (const k of requeridas) {
    if (!process.env[k]) return falha(503, "config_ausente", `${k} não configurado`);
  }
  ...
  if (!apurado) return falha(422, "sem_vencedor", "nenhum lance único nesta edição");
  ...
  try {
    ...
    return falha(502, "envio_falhou", "falha ao submeter consolidação: " + (err?.shortMessage || err?.message));
  }

  // 9. Dropped tx handling (ITEM 4.2): espera com timeout; NUNCA reenvia auto.
  const receipt = await provider.waitForTransaction(tx.hash, 1, timeoutMinerMs).catch(() => null);
  if (!receipt) {
    return {
      status: 202,
      corpo: {
  ...
  // MC94.3 — com `pontua: false` (ESPECIAL-*) o null é de propósito; e mesmo que
  // alguém a repontue, `registrarPontuacaoRodada` recusa-a (guarda central).
  ...
  // 2026-09-25). A guarda central (MC94.3) está em registrarPontuacaoRodada; esta
  // fica como segunda linha — redundância aceite pelo operador.
  const pontua = !EDICAO_ESPECIAL_RE.test(edicaoId);
  let pontuacao = null;
  if (!pontua) {
    console.info("[consolidacao] edição especial consolidada SEM pontuar o torneio", edicaoId);
  } else {
    try {
      pontuacao = await registrarPontuacaoRodada(edicaoId, lances);
    } catch (err) {
      console.error("[consolidacao] pontuação falhou (consolidação mantém-se)",
